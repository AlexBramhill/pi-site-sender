...
import {
  ArrivalsClientResponse,
  ArrivalsClientResponseSchema,
} from "../schemas/arrivials";
...
export const getArrivals = async (
  stationId: string
): Promise<ArrivalsClientResponse> => {
  const params = new URLSearchParams({
    app_key: config.TFL_API_KEY,
    stationId,
  });
  ...
  const response = await fetch(
    `https://api.tfl.gov.uk/StopPoint/${stationId}/Arrivals?${params}`
  );
  ...
  const data = await response.json();
  return ArrivalsClientResponseSchema.parse(data);
};
